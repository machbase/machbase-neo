...
Change it to new path where you want to store the database files. If the folder is newly created and has no database files, machbase-neo will create them automatically.
...
- `prefDir(subdir)` : Ger user's preference directory, On Linux and macOS, it returns $HOME/.config/{subdir}
