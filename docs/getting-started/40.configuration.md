...
The default value of `MachbaseHome` is `${execDir()}/machbase_home` which is a sub-directory of where `machbase-neo` executable file is.

Change it to new path where you want to store the database files. If the new folder is newly created and has no database files, machbase-neo will create them automatically.
...
- flag(A, B) : get value of command line flag 'A'. if not specified, apply B as value.
- env(A, B) : get value of Envrionment variable 'A'. if not specified, appy B as value
- execDir()  : get directory path where executable file is.
- more functions are available from `machbase/booter` framework.
...
> Security
>
> Before allow remote access, Should change `SYS`'s default password from `manager` to your own.
