...
Now we can use the output of the script as input of `machbase-neo shell`.

```sh
sh wave.sh | machbase-neo shell import --timeformat=s EXAMPLE
```

CSV lines that are generated by shell script are being processed in `machbase-neo shell import` and "import" into `EXAMPLE` table.
...
requires relatively less lines of code compare to HTTP.
...
Command blow generates `chart.html` file that contains a chart.
