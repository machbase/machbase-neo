...
As soon as executing the script above the `mosquitto_sub` prints out tge messages on the screen.
...
First it gets current time to apply the timestamp with `ts := times.now()`, since the message doesn't hasve a timestamp.
