...
7. Let's try to change `OUTPUT( CSV() )` to `OUTPUT( JSON() )`, save and execute again.
    ...
    ```
8. If you are developing an data visualization application, it may be helpful to know that *tql* JSON output suppors transpose the records in columns instead of rows. Apply `OUTPUT( JSON( transpose(true) ) )` and invoke it again. The result JSON contains *cols* array.
